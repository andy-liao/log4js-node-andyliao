...
function DateRollingFileStream(filename, pattern, options, now,resDays) {
    debug("Now is " + now);
...
DateRollingFileStream.prototype.roll = function(filename, callback) {
    ...
    async.series([
    	deleteOldestReservedFile,
        deleteAnyExistingFile,
        renameTheCurrentFile,
        openANewFile
    ], callback);
    
    function deleteOldestReservedFile(cb) {
		var deleteFile = filename +  format.asString(that.pattern, new Date(Date.now() - that.reserveDays * 24 * 3600 * 1000));
       	debug("delete oldest reservedFile:" + deleteFile);
